...
OPTIONS = [('data','Obtener productos'),('images',u'Actualizar imágenes'),('price_stock','Actualizar precio y stock')]
result = 6450

_logger = logging.getLogger(__name__)

class StockToyApiWizard(models.TransientModel):
    _name = 'stock.toy.api.wizard'
    _description = 'Toys Api - Consulta de productos'
    ...
    sucursal_id = fields.Many2one('stock.sucursal.toys', string=u'Sucursal', required=True)
    ...
    def create_mensaje(self,res,sucursal,mensaje):
    ...
    def update_images(self):
        ...
        for sucursal in self.sucursal_id:
            ...
            if isinstance(res, list):
                msn = self.create_mensaje(res,sucursal,mensaje)
                self.description = msn
            else:
                self.description = 'Error al actualizar los productos'
                break
        return {
            u'name': u'Webservice Toys',
            u'type': u'ir.actions.act_window',
            u'view_mode': u'form',
            u'target': u'new',
            u'res_model': u'stock.toy.api.wizard',
            u'res_id': self.id
        }
